...
this media, online communities formed and successfully combine
...
\R{} via \code{install.packages("foo",
...
\item Project categorization: Administrators may categorize their
  ...
  to quickly find what they are looking using the \tab{Project Tree}
...
pre-defined directories namely \file{www} and \file{pkg} get
...
PC. Typically, for a project named \code{foo} the repository is
...
\file{pkg} directory or a subdirectory if it and commit the changes to
...
\file{pkg/R}, \file{pkg/man}, \ldots{}) or, alternatively, a 
...
\R{}-Forge automatically examines daily the \file{pkg} directory of
...
\code{install.packages("foo",
  repos="http://R-Forge.R-project.org")}. Furthermore, logs of the
build and check process on different platforms can be examined in the
\tab{R Packages} tab by the developers.
